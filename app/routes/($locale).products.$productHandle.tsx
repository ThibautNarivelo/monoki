...
export default function Product() {
  const {product, shop, recommended, variants, analytics} =
  ...
  const onPrevClick = () => {
    setCurrentImg(currentImg === 0 ? media.nodes.length - 1 : currentImg - 1);
  };

  const onNextClick = () => {
    setCurrentImg(currentImg === media.nodes.length - 1 ? 0 : currentImg + 1);
  };
  return (
    <>
      <section className="pt-[32px] px-[1.1rem]">
        <Heading>{title}</Heading>
        <div className="w-full h-full flex flex-col lg:flex-row justify-between items-start">
          {descriptionHtml && (
            ...
            <Suspense
              fallback={
                <Forms
                  variants={[]}
                  product={product}
                  productAnalytics={analytics.products[0]}
